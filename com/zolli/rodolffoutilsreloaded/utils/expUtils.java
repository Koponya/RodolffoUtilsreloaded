...
public class expUtils {
	...
	// Initialise the xp lookup table. Basing this on observations noted in
	// https://bukkit.atlassian.net/browse/BUKKIT-47
	// 7 xp to get to level 1, 17 to level 2, 31 to level 3...
	// At each level, the increment to get to the next level increases
	...
	public void awardExperience(Player player, int xp)
	{
		...
		player.giveExp(xp);
		// System.out.println("gave " + player.getName() + " " + xp +
		// " exp, new totalexp = " + player.getTotalExperience() +
		// " new getExp() = " + player.getExp());
		int newLevel = getRealLevel(player);
		if (curLevel != newLevel)
		{
			// recalculate the player's actual level and distance through the
			// level
			// System.out.println("levelled from " + curLevel + " to " +
			// newLevel);
